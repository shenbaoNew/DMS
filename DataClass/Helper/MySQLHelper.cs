...
namespace DMS.Helper {
    ...
    public class MySQLHelper {
        ...
        public MySQLHelper(string server, string db, string user, string pwd)
            : this(server, db, user, pwd, "3306") {
        }

        public MySQLHelper(string server,string db,string user,string pwd,string port) {
            this.ConnectionString = string.Format("server={0};user id={1};password={2};database={3};port={4};Charset=utf8", server, user, pwd, db, port);
        }
        ...
        public bool TestConn() {
            ...
            ExecuteNonQuery(sql, new MySqlParameter[] { });
            return true;
        }
