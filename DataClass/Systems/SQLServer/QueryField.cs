    ...
    public class QueryFieldCollection : BindingList<QueryField>
    {
            ...
            if (e.ListChangedType == ListChangedType.ItemChanged)
            {
                ...
                switch (e.PropertyDescriptor.Name)
                {
                    ...
                    case "ColAlias":
                        ...
                        {
                            if (field != f && f.ColAlias == field.ColAlias)
                            {
                                f.ColAlias = CreateUniqueAlias(f);
                                break;
                            }
                        }
                        break;
                    ...
                    case "GroupBy":
                        if (f.GroupBy != Aggregate.GroupBy && string.IsNullOrEmpty(f.ColAlias))
                        {
                            f.ColAlias = CreateUniqueAlias(f);
                        }
        ...
        string CreateUniqueAlias(QueryField f)
        {
            for (int i = 1; true; i++)
            {
                ...
                foreach (QueryField field in this)
                {
                    if (field != f && string.Compare(alias, field.ColAlias, true) == 0)
                    {
                        duplicate = true;
                        break;
                    }
